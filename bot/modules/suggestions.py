...
class Suggestions(commands.Cog):
 ...
 async def on_message(self,message):
  ...
  cfg=await settings(message.guild.id); ch=int(cfg.get('suggestion_channel_id','0') or 0)
  if message.channel.id != ch: return
  content=message.content.strip(); att=[a.url for a in message.attachments]
  if att: content+='\n\n'+'\n'.join(att)
  if not content: return
  sid=await execute('INSERT INTO suggestions (guild_id,discord_id,discord_name,content,status,created_at,updated_at) VALUES (?,?,?,?,?,?,?)',(str(message.guild.id),str(message.author.id),str(message.author),content,'analysis',now(),now()))
  ...
  await execute('UPDATE suggestions SET message_id=? WHERE id=?',(str(sent.id),sid)); await log('suggestion_created', {'guild_id':str(message.guild.id),'id':sid,'user':message.author.id})
